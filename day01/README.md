...
- 追加で確認した入力例：`--name "Taro" --repeat 2 --format json` を実行し、JSON形式で2件出力されることを確認した。
- 発生したエラーと対処：`--name`を指定せずに実行すると、必須項目がないことを示すエラーと終了コード`2`が返った。`--name`を指定することで正常に実行できた。
